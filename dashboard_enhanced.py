...
if run_btn:
    ...
    # Pipeline Stages
    PIPELINE_STAGES = ["Intake", "Schema", "Transform", "Validate", "Analyze", "Report"]
        ...
        with st.status("⚙️ **Stage 3: Data Transformation**", expanded=True) as status:
            ...
            add_agent_message(
                ...
                findings=[{
                    ...
                    "impact": f"Cost coverage: {(stats_imp.get('with_cost', 0)/len(all_records)*100):.1f}%" if len(all_records) > 0 else "N/A"
                }]
            )
    ...
    # ========== STAGE 5: ANALYSIS ==========
    with detail_placeholder:
        with st.status("📈 **Stage 5: Strategic Analysis**", expanded=True) as status:
            current_stage = 4
            with stage_placeholder:
                render_progress_pipeline(current_stage, PIPELINE_STAGES)
            ...
            **Strategic Analysis Summary:**
            ...
            status.update(label="✅ Stage 5 Complete: Analysis Done", state="complete")

    # ========== STAGE 6: REPORT GENERATION ==========
    with detail_placeholder:
        with st.status("📝 **Stage 6: Report Generation**", expanded=True) as status:
            current_stage = 5
            with stage_placeholder:
                render_progress_pipeline(current_stage, PIPELINE_STAGES)
            ...
            status.update(label="✅ Stage 6 Complete: Report Ready", state="complete")
...
if st.session_state.processing_complete and st.session_state.baseline_data is not None:
    ...
    advanced_mode = st.checkbox(
        "Advanced mode",
        value=False,
        help="Show technical logs, references, and raw tables."
    )
    ...
    if "💬 Process Notes" in tab_map:
        with tab_map["💬 Process Notes"]:
            st.subheader("💬 Process Notes")
            st.caption("Plain-language timeline of how the system processed your data.")

            show_technical = st.checkbox(
                "Show technical agent names",
                value=False,
                help="When off, roles are shown instead of agent names.",
                key="process_notes_show_technical"
            )
            show_types = st.checkbox(
                "Show message types",
                value=False,
                help="Show labels like success, alert, or handoff.",
                key="process_notes_show_types"
            )

            if st.session_state.agent_communications:
                filter_choice = st.selectbox(
                    "Show",
                    ["All", "Alerts only", "Findings only", "Success only"],
                    key="process_notes_filter"
                )

                for msg in st.session_state.agent_communications:
                    msg_type = msg.get("type")
                    if filter_choice == "Alerts only" and msg_type not in ("alert", "warning"):
                        continue
                    if filter_choice == "Findings only" and msg_type != "finding":
                        continue
                    if filter_choice == "Success only" and msg_type != "success":
                        continue

                    persona = AGENT_PERSONAS.get(msg.get("agent_key"))
                    display_name = msg["agent_name"]
                    if not show_technical and persona:
                        display_name = persona.role

                    type_label = f" ({msg_type})" if show_types and msg_type else ""
                    st.markdown(f"- {msg['timestamp']} — **{display_name}**: {msg['message']}{type_label}")
            else:
                st.info("Run the pipeline to see agent communications.")

    if "🔍 Issues & References" in tab_map:
        with tab_map["🔍 Issues & References"]:
            st.subheader("🔍 Issues & References")
            st.caption("Plain-language issues with optional source references.")

            if st.session_state.findings_log:
                # Summary metrics
                col1, col2, col3, col4 = st.columns(4)
                findings = st.session_state.findings_log
                with col1:
                    st.metric("Total Findings", len(findings))
                with col2:
                    st.metric("Success", len([f for f in findings if f['severity'] == 'success']))
                with col3:
                    st.metric("Warnings", len([f for f in findings if f['severity'] == 'warning']))
                with col4:
                    st.metric("Critical", len([f for f in findings if f['severity'] == 'critical']))

                st.markdown("---")

                # Findings list (plain language)
                for finding in findings:
                    severity_colors = {
                        'success': ('#d4edda', '#155724', '✅'),
                        'info': ('#cce5ff', '#004085', 'ℹ️'),
                        'warning': ('#fff3cd', '#856404', '⚠️'),
                        'critical': ('#f8d7da', '#721c24', '🚨')
                    }
                    bg, text_color, icon = severity_colors.get(finding['severity'], ('#e2e3e5', '#383d41', '•'))

                    st.markdown(f"""
                    <div style="background: {bg}; border-radius: 8px; padding: 15px; margin: 10px 0;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-weight: bold; color: {text_color};">
                                {icon} {finding['type']}
                            </span>
                            <span style="color: #666; font-size: 0.8em;">{finding['timestamp']}</span>
                        </div>
                        <div style="margin: 10px 0; color: {text_color};">
                            <strong>What we saw:</strong> {finding['description']}
                        </div>
                        <div style="margin-top: 8px; font-size: 0.9em; color: {text_color};">
                            <strong>Why it matters:</strong> {finding['impact']}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    with st.expander("Show source reference"):
                        st.code(finding['reference'])
            else:
                st.info("Run the pipeline to see findings.")

            with st.expander("Advanced: Agent Audit Trail"):
                st.caption("Complete decision audit for compliance and review.")

                audit_tab1, audit_tab2, audit_tab3 = st.tabs(["📦 Intake", "🧠 Schema Logic", "✅ Standardization"])

                with audit_tab1:
                    st.markdown("#### Intake Agent Diagnostics")
                    if 'intake' in audit_logs and audit_logs['intake']:
                        intake_rows = []
                ...
                with audit_tab2:
                    st.markdown("#### Schema Agent Mapping Decisions")
                    if 'schema' in audit_logs and not audit_logs['schema'].empty:
                ...
                with audit_tab3:
                    ...
                    if 'standardizer' in audit_logs and not audit_logs['standardizer'].empty:
    ...
    if "🧭 Consultant View" in tab_map:
        with tab_map["🧭 Consultant View"]:
            ...
            if ('schema' in audit_logs) and (schema_total is None or schema_skipped is None) and not audit_logs['schema'].empty:
            ...
            if not files_processed:
                ...
                intake_next = (
                    "Enable Advanced mode to review files that failed to read."
                    if not advanced_mode
                    else "Open Advanced Audit to review files that failed to read."
                )
            stage_items.append({
            ...
            if schema_skipped:
                schema_next = (
                    "Enable Advanced mode to review skipped sheets and confirm column mapping."
                    if not advanced_mode
                    else "Review skipped sheets in Advanced Audit to confirm column mapping."
                )
            else:
                schema_next = None
            stage_items.append({
            ...
            if (qa_outliers_flagged or 0) > 0 or (qa_critical or 0) > 0:
                validate_next = (
                    "Enable Advanced mode to review flagged anomalies."
                    if not advanced_mode
                    else "Review flagged anomalies in Issues & References."
                )
            stage_items.append({
